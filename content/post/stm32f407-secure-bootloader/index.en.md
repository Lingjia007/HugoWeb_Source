...
## 10. Security Chain Overview

**Firmware package build side:**

1. Original firmware → SHA256 (appended to end)
...
1. `.iap.bin` → HMAC-SHA256 verification (DevKey)
2. → Hardware compatibility check
3. → Anti-rollback check
4. → HKDF derive AES key (Salt + DevKey + UID)
5. → AES decrypt → write to Flash
6. → SHA-512 streaming hash → Ed25519 signature verification
7. → Read back Flash SHA-256 check

The complete security chain forms a **chain of trust**: HMAC ensures package source is trusted → encryption ensures content confidentiality → signature ensures integrity → anti-rollback ensures version cannot be downgraded → SHA256 read-back check ensures Flash write correctness.
