...
C 語言提供的複合型別工具極少：基本型別、陣列、結構體、聯合體、列舉、指標。其中真正能承載設計複雜度的只有兩個：**結構體**和**指標**。

- **結構體**是 C 中唯一的資料聚合工具——把多個屬性綁在一起，形成「物件」的雛形
- **指標**是 C 中唯一的間接參照工具——透過位址操作任意物件，實現「多態」的根基

沒有結構體，資料只能散落為全域變數；沒有指標，函式只能操作固定物件，無法實現多實例、多態、回呼。C++ 的 `class`、`virtual`、`interface` 本質上都是這兩者的語法糖。

### 0.2 片上外設位址映射：結構體指標的天然舞台

打開任何一款 ARM Cortex-M 晶片的標準外設庫或 HAL 庫，你會看到這樣的程式碼：

**STM32 HAL（`stm32f407xx.h`）：**

```c
typedef struct {
    __IO uint32_t SR;         // 狀態暫存器，偏移 0x00
    __IO uint32_t DR;         // 資料暫存器，偏移 0x04
    __IO uint32_t BRR;        // 波特率暫存器，偏移 0x08
    __IO uint32_t CR1;        // 控制暫存器1，偏移 0x0C
    __IO uint32_t CR2;        // 控制暫存器2，偏移 0x10
    __IO uint32_t CR3;        // 控制暫存器3，偏移 0x14
    // ... 更多暫存器
} USART_TypeDef;

#define USART1  ((USART_TypeDef *)0x40011000)  // 強制轉換！
#define USART2  ((USART_TypeDef *)0x40004400)
#define USART6  ((USART_TypeDef *)0x40011400)
```

**這裡發生了什麼？** 晶片設計者在硬體層面把外設暫存器按固定偏移排列在一段連續位址空間中。C 語言的結構體成員預設也按宣告順序排列，編譯器按成員大小和對齊規則計算偏移——當結構體成員的宣告順序和型別與硬體暫存器佈局一致時，結構體的記憶體佈局與硬體暫存器佈局**完全吻合**。

然後，把外設基底位址**強制轉換**為指向該結構體的指標，就可以用 `USART1->CR1` 這樣的方式直接讀寫硬體暫存器。

> [!NOTE]
> 現代 C/C++ 編輯器或 IDE（VS Code + clangd、STM32CubeIDE、Keil MDK 等）對結構體成員有**智能補全**支援——輸入 `USART1->` 時會自動列出 `SR`、`DR`、`BRR`、`CR1` 等所有成員，而魔數偏移 `0x0C` 則無任何提示。這意味著結構體指標映射不僅編譯時安全，**編寫時也安全**——拼寫錯誤在輸入階段就被消滅，而非等到執行時才暴露。

### 0.3 為什麼這不是「駭客技巧」而是天然設計？

| 優勢               | 說明                                                                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **零開銷**         | `USART1->CR1` 編譯後就是一條 `LDR`/`STR` 指令加上固定偏移，和直接寫 `*(volatile uint32_t *)0x4001100C` 完全等價，沒有任何額外開銷                     |
| **型別安全**       | `USART1` 是 `USART_TypeDef *` 型別，不可能誤寫成 `GPIOA->CR1`——編譯器會報錯，因為 `GPIO_TypeDef` 沒有 `CR1` 成員                                      |
| **可讀性**         | `usart_baudrate_set(USART0, 115200)` 比 `*(volatile uint32_t *)(0x40011000 + 0x08) = 0x1A0B` 人類可讀得多                                             |
| **編譯時檢查**     | 結構體成員名寫錯 → 編譯錯誤；魔數偏移寫錯 → 執行時才炸                                                                                                |
| **多實例天然支援** | 同一個 `USART_TypeDef` 結構體，不同的基底位址 = 不同的外設實例。`usart_init(USART0)` 和 `usart_init(USART1)` 共用同一份程式碼——這不就是 `me` 指標嗎？ |

### 0.4 從硬體映射到軟體 OOP 的統一邏輯

晶片廠商用結構體指標映射硬體暫存器，和我們用結構體指標實現面向物件，**底層邏輯完全一致**：

| 硬體映射                            | 軟體 OOP                     | 統一邏輯                             |
| ----------------------------------- | ---------------------------- | ------------------------------------ |
| `USART_TypeDef` 結構體              | `struct led` 結構體          | 結構體聚合資料，形成「物件」         |
| `USART1` 基底位址強制轉換           | `&red_led` 取位址            | 指標指向具體實例                     |
| `USART1->CR1 = val`                 | `me->pin = 5`                | 透過指標存取實例成員                 |
| `usart_init(USART0)`                | `led_init(&red_led)`         | 函式透過指標操作不同實例             |
| `USART_TypeDef` 在所有 USART 間復用 | `led_on()` 在所有 LED 間復用 | 同一份程式碼操作不同實例——多態的雛形 |

> [!IMPORTANT]
> 結構體 + 指標不是 C 語言的「補丁」，而是 C 語言與硬體互動的**原生方式**。晶片廠商用它映射暫存器，內核開發者用它實現多態，我們用它構建平台抽象——一脈相承。
